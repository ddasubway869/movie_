...
app.get("/api/user/settings", requireAuth, (req, res) => {
  ...
  res.json({ hasApiKey: !!(user?.torbox_api_key), maskedKey: user?.torbox_api_key ? "••••••••" + user.torbox_api_key.slice(-4) : null });
});

app.put("/api/user/settings", requireAuth, (req, res) => {
  const { torboxApiKey } = req.body || {};
  if (torboxApiKey === undefined)
    return res.status(400).json({ error: "torboxApiKey is required" });
